...
@cluster_metric
def compute_tss(clustered_vectors, metric=euclidean_distance, param=None):
...
def compute_bss_indirectly(clustered_vectors, metric=euclidean_distance, param=None):
    rss, _ = compute_rss(clustered_vectors, metric)
    tss, _ = compute_tss(clustered_vectors, metric)
    bss = tss - rss
    return bss, []

@cluster_metric
def compute_bss(clustered_vectors, metric=euclidean_distance, param=None):
...
def compute_rss(clustered_vectors, metric=euclidean_distance, param=None):
    rss = 0
    centroids = get_cluster_centroids(clustered_vectors)
    cluster_rss_list = []
    ...
    return rss, cluster_rss_list

@cluster_metric
def compute_aic(clustered_vectors, metric=euclidean_distance, param=None):
...
@cluster_metric
def compute_bic(clustered_vectors, metric=euclidean_distance, param=None):
    M = len(clustered_vectors[0][0])
    n = len(clustered_vectors)
    all_vectors = []
    for cluster in clustered_vectors:
    ...
    rss, _ = compute_rss(clustered_vectors, metric)
    bic = rss + M * n * numpy.log(data_points)
    return bic, []

@cluster_metric
def compute_tuneable_aic(clustered_vectors, metric=euclidean_distance, param=2.0):
    M = len(clustered_vectors[0][0])
    n = len(clustered_vectors)
    rss, _ = compute_rss(clustered_vectors, metric=euclidean_distance)
    aic = rss + param * M * n
    return aic, []
...
@cluster_metric
def compute_pseudo_f(clustered_vectors, metric=euclidean_distance, param=None):
    ...
    if (num_clusters == 1) or (num_clusters == num_obs):
        return 0
    bss, _ = compute_bss(clustered_vectors, metric)
    rss, _ = compute_rss(clustered_vectors, metric)
...
def compute_pseudo_f_indirecly(clustered_vectors, metric=euclidean_distance, param=None):
    ...
    if (num_clusters == 1) or (num_clusters == num_obs):
        return 0
    bss, _ = compute_bss_indirectly(clustered_vectors, metric)
    rss, _ = compute_rss(clustered_vectors, metric)
    pseudo_f = (bss / (num_clusters - 1)) / (rss / (num_obs - num_clusters))
    return pseudo_f, []
...
@cluster_metric
def compute_sk2_silhouette(clustered_vectors, metric=euclidean_distance, param=None):
...
@cluster_metric
@single_cluster_metric
def compute_average_silhouette_score(clustered_vectors, metric=euclidean_distance, param=None):
...
def main():
    ...
    print "average sk2 silhouette is ", compute_sk2_silhouette(clustered_vectors)
    print "AIC is ", compute_aic(clustered_vectors)
    print "Tuneable AIC is ", compute_tuneable_aic(clustered_vectors, param=4)
